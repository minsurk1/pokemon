...
function getCardImageByNameAndTier(
  name: string,
  tier: number,
  owned: boolean
): string {
...
async function openCardPackApiCall(
  userId: string,
  packType: string
): Promise<CardData[]> {
...
function Inventory({ inventory, setInventory }: InventoryProps) {
  ...
  const openCardPack = async (index: number) => {
    ...
    try {
      const drawnCardsFromServer = await openCardPackApiCall(
        userId,
        cardPack.type
      );
