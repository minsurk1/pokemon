...
router.post("/create", isAuthenticated, async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (existing) return res.status(400).json({ message: "같은 이름의 덱이 이미 존재합니다." });
...
// 2) 카드 추가
router.post("/add-card", isAuthenticated, async (req: AuthenticatedRequest, res: Response) => {
  const userId = req.user?._id;
  ...
  if (!deck) return res.status(404).json({ message: "덱을 찾을 수 없음" });

  // 덱 최대 30장 확인
  if (deck.cards.length >= 30) return res.status(400).json({ message: "덱은 최대 30장입니다." });
...
// 3) 카드 제거
router.post("/remove-card", isAuthenticated, async (req: AuthenticatedRequest, res: Response) => {
  const userId = req.user?._id;
  const { deckId, cardId } = req.body;
...
// 4) 덱 조회
router.get("/my-decks", isAuthenticated, async (req: AuthenticatedRequest, res: Response) => {
  const userId = req.user?._id;
