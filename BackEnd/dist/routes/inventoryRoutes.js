...
const UserPack_1 = __importDefault(require("../models/UserPack")); // 유저가 가진 카드팩 데이터 모델
const UserCard_1 = __importDefault(require("../models/UserCard")); // 유저가 가진 카드 데이터 모델
const Card_1 = __importDefault(require("../models/Card")); // 전체 카드 정보 모델
...
function getRandomTier(probabilities) {
    ...
    for (const tier in probabilities) {
        ...
        if (rand <= cumulative)
            return +tier; // 해당 구간에 걸리면 tier 반환
    }
...
router.post("/open-pack", isAuthenticated_1.isAuthenticated, async (req, res) => {
    const session = await mongoose_1.default.startSession(); // 트랜잭션 세션 시작
    try {
        const userId = req.user?.id; // 로그인한 유저 id (isAuthenticated 미들웨어가 넣어줌)
        const { packType } = req.body; // 어떤 종류의 팩을 열지 클라이언트에서 전달
        if (!userId)
            return res.status(401).json({ message: "인증 실패" });
        let drawnCards = []; // 뽑은 카드들을 저장할 배열
        await session.withTransaction(async () => {
            // 1) 유저팩 수량 차감 (packType에 맞는 UserPack 찾아서 quantity -1)
            const pack = await UserPack_1.default.findOneAndUpdate({ user: userId, packType, quantity: { $gt: 0 } }, { $inc: { quantity: -1 } }, { new: true, session });
            if (!pack)
                throw new Error("보유한 카드팩이 없습니다."); // 없는 경우 트랜잭션 롤백
            // 2) 카드 전체 불러오기
            const allCards = await Card_1.default.find({}).session(session);
            ...
            for (let i = 0; i < 5; i++) {
                ...
                if (tierCards.length === 0)
                    continue; // 등급 카드 없으면 스킵
                const randomCard = tierCards[Math.floor(Math.random() * tierCards.length)]; // 랜덤 1장
                drawnCards.push(randomCard);
                // UserCard 컬렉션에 저장 (이미 있으면 count+1, 없으면 새로 생성)
                await UserCard_1.default.findOneAndUpdate({ user: userId, card: randomCard._id }, { $inc: { count: 1 }, $set: { owned: true } }, { upsert: true, session });
            }
        });
