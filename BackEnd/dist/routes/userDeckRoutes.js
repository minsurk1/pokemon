...
const UserDeck_1 = __importDefault(require("../models/UserDeck"));
const isAuthenticated_1 = require("../middleware/isAuthenticated");
const router = express_1.default.Router();
// ✅ 덱 불러오기 (로그인한 유저 기준)
router.get("/single", isAuthenticated_1.isAuthenticated, async (req, res) => {
    try {
        const user = req.user;
        if (!user?._id) {
        ...
        const deck = await UserDeck_1.default.findOne({ user: userObjectId })
            .populate({
            path: "cards.card",
            model: "Card",
        ...
        const formattedDeck = {
            ...
            cards: (deck.cards || []).map((entry) => {
                // ✅ populate 여부에 따라 분기
                const c = typeof entry.card === "object" && entry.card !== null
...
// ✅ 덱 저장
router.post("/single/save", isAuthenticated_1.isAuthenticated, async (req, res) => {
    try {
        const user = req.user;
        if (!user?._id) {
        ...
        const formattedCards = cards.map((c) => ({
            card: new mongoose_1.default.Types.ObjectId(String(c.id || c.cardId)),
            name: c.name,
        ...
        let deck = await UserDeck_1.default.findOne({ user: userObjectId });
        if (!deck) {
            deck = new UserDeck_1.default({
                user: userObjectId,
