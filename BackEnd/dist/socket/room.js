...
const battle_1 = require("./battle"); // ✅ 전투 초기화 연결
// ✅ 모든 방 상태 저장소
exports.rooms = {};
/**
...
function roomHandler(io, socket) {
    ...
    socket.on("startGame", ({ roomCode }) => {
        ...
        const allReady = players.every((id) => room.ready[id]);
        if (!allReady) {
        ...
        // ✅ 전투 초기화 호출
        (0, battle_1.initializeBattle)(io, roomCode, room);
    ...
    socket.on("sendDeck", ({ roomCode, deck }) => {
        const room = exports.rooms[roomCode];
        if (!room || !deck || !Array.isArray(deck))
            return;
        ...
        console.log(`📥 덱 수신 from ${socket.id}:`, deck.map((c) => ({ id: c.id, name: c.name, atk: c.attack })));
    });
}
